...
| Variant generation | Provider-agnostic rephraser (default: Gemini) — paraphrases responses and phrases |
...
| Auth | Delegated to org identity provider via Auth Adapter. VARIO issues JWT with role claim after org validates. |
...
Integration Adapters  → AuthAdapter, HRMSAdapter, ITSMAdapter, AdmissionsAdapter
Mock Internal Systems → 4x FastAPI mock services standing in for real backends
PostgreSQL (Neon)     → single source of truth for everything, no in-memory cache
```

The whole point of this layering: **swap mocks for real backends later without touching Role Pack or Core Engine code.**
...
| `auth` | Thin adapter — forwards credentials to org's identity provider, issues VARIO JWT on success. No registration, email verification, or password management. |
...
| `integration-adapters` | One per backend system (Auth, HRMS, ITSM, Admissions). Only code that knows it's talking to a mock. |
...
| `rephraser-service` | Generates 4 paraphrased response variants on template save. Also generates training phrase variants ("Generate similar") on demand. |
...
Admin adds an intent + phrases + response template → Admin CRUD persists it and flags `needs_retrain = true` → **synchronously** calls the Rephraser Service → LLM paraphrases the base text into 4 response variants (placeholders preserved and validated) → all 5 versions (original + 4) returned to the console for admin review/edit/delete. Admin can also click "Generate similar" on training phrases to get 8 new phrase variations to review and accept. Retraining is a **separate, manual step**: admin clicks "Retrain," the service pulls that role pack's intents/phrases, converts to Rasa YAML, runs `rasa train`, reloads the live Rasa instance, and clears the retrain flag.

---

## 12. Data model (9 entities — ADD.md)

| Entity | Key notes |
|---|---|
| `users` | Lightweight cache of org users — upserted on first login. Has many conversations/phrases/templates/audit entries. |
...
| `intents` | scoped by `role_pack`; `needs_retrain` flag; `intent_type` (`dynamic_workflow` vs `static_faq`) |
| `training_phrases` | belongs to `intents`, authored by admin users |
| `response_templates` | `template_key`, `base_text`, `allow_rephrasing` flag, `available_variables` (array) |
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/api/auth/login` | Login via org auth adapter → VARIO JWT |
...
| POST | `/api/admin/intents/:id/generate-phrases` | Generate training phrase variants via Rephraser Service |
...
| GET | `/api/admin/conversations[/:id/messages]` | View role-pack conversation logs / transcript (FR-19) |
...
- English only, web only, no push notifications — don't scope-creep into these.
...
- User provisioning and password management are the org's responsibility, not VARIO's.
...
- **Auth depth:** Auth is now explicitly delegated to the organization's identity provider via an Auth Adapter. VARIO does not handle user registration, email verification, or password reset functionality.
- **ERD file:** The standalone `vario_database_erd.html` has been removed; the 9-entity data model is now described inline in the ADD.
- **Open questions still genuinely unresolved:** the exact DB schema strategy for isolating role-pack config while sharing the sessions table.
